...
import com.rec.calls.receivers.TelephonyManagerPhoneStateReceiver;
...
public class MainService extends Service {
	...
	private final SharedPreferences.OnSharedPreferenceChangeListener mOnSharedPreferenceChangeListener = (sharedPreferences, s) -> {
		...
		if (s.equals (EnvironmentApplication.SP_KEY_RECORD_INCOMING_CALLS)) {
		...
		if (s.equals (EnvironmentApplication.SP_KEY_RECORD_OUTGOING_CALLS)) {
	...
	private final TelephonyManagerPhoneStateReceiver mCaPhoneNumberReceiver = new TelephonyManagerPhoneStateReceiver();
	...
	public void onCreate () {
		...
		if (mNotificationManager != null) {
			CharSequence contentTitle = getString(R.string.txt_running), contentText = getString (R.string.app_name) + getString(R.string.txt_is_active);
		...
		try {
			registerReceiver (mCaPhoneNumberReceiver, new IntentFilter (TelephonyManager.ACTION_PHONE_STATE_CHANGED));
		} catch (Exception e) {
			LogUtils.LOGE (TAG, e.getMessage ());
			LogUtils.LOGE (TAG, e.toString ());
			e.printStackTrace ();
	...
	public void onDestroy () {
		...
		try {
			unregisterReceiver (mCaPhoneNumberReceiver);
		} catch (Exception e) {
			LogUtils.LOGE (TAG, e.getMessage ());
			LogUtils.LOGE (TAG, e.toString ());
			e.printStackTrace ();
